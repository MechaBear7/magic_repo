        ...
        with open(data_file, "r", encoding="utf-8") as f:
            ...
            for article in json_data["data"]:
                ...
                for question in article["paragraphs"][0]["qas"]:
                    ...
                    Data[idx] = {"id": q_id, "title": title, "context": context, "question": ques, "answers": {"text": text, "answer_start": answer_start}}
                    idx += 1
        return Data
...
def test_collote_fn(batch_samples):
    ...
    for i in range(len(batch_data["input_ids"])):
        ...
        offset_mapping[i] = [o if sequence_ids[k] == 1 else None for k, o in enumerate(offset)]
...
def test_loop(dataloader, dataset, model):
    ...
    theoretical_answers = [{"id": dataset[s_idx]["id"], "answers": dataset[s_idx]["answers"]} for s_idx in range(len(dataset))]
    predicted_answers = []
    for s_idx in tqdm(range(len(dataset))):
        ...
        if len(answers) > 0:
            ...
            predicted_answers.append({"id": example_id, "prediction_text": best_answer["text"], "answer_start": best_answer["start"]})
        else:
