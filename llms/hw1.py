...
def train_loop(dataloader, model, optimizer, lr_scheduler, epoch, steps, total_loss, tf_writer):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        progress_bar.set_description(f"loss: {total_loss / (idx + finish_batch_num + 1):>7f}")
...
def test_loop(dataloader, model, steps, tokenizer, bleu, tf_writer):
    ...
    for batch in tqdm(dataloader):
        batch = batch.to(device)
        with torch.no_grad():
            generated_tokens = model.generate(batch["input_ids"], attention_mask=batch["attention_mask"], max_length=512)
...
def train():
    ...
    def collate_function(batch):
        batch_inputs = [f'问题：{sample["question"]}{tokenizer.sep_token}原文：{sample["context"]}' for sample in batch]
        batch_targets = [f'答案：{sample["answer"]}{tokenizer.eos_token}' for sample in batch]

        batch_data = tokenizer(batch_inputs, padding=True, truncation=True, max_length=512, return_tensors="pt")

        batch_labels = tokenizer(batch_targets, padding=True, truncation=True, max_length=512, return_tensors="pt")["input_ids"]

        decoder_input_ids = batch_labels[:, :-2]
        decoder_input_ids = F.pad(decoder_input_ids, (0, 2), value=tokenizer.pad_token_id)
    ...
    train_dataloader = DataLoader(train_data, batch_size=32, shuffle=True, collate_fn=collate_function)
    val_dataloader = DataLoader(val_data, batch_size=32, shuffle=False, collate_fn=collate_function)
    ...
    for t in range(epoch_num):
        ...
        total_loss, steps = train_loop(train_dataloader, model, optimizer, lr_scheduler, t + 1, steps, total_loss, tf_writer)
...
def infer():
    ckpt = "/home/helei.yang/codes/llms/output/hw1/ckpt/epoch_30_valid_bleu_100.00_model_weights.bin"
    model_name = "uer/t5-base-chinese-cluecorpussmall"
    ...
    tokenized_input = tokenizer(model_input, padding=True, truncation=True, max_length=512, return_tensors="pt")
    tokenized_input.to(device)

    model.eval()
    with torch.no_grad():
        generated_tokens = model.generate(tokenized_input["input_ids"], attention_mask=tokenized_input["attention_mask"], max_length=512)
    generated_texts = tokenizer.decode(generated_tokens[0], skip_special_tokens=True)
