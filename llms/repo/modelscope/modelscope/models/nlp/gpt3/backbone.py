...
class GPT3SelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, ltor_mask, is_infer=False):
        ...
        (mixed_query_layer, mixed_key_layer, mixed_value_layer) = self._split_tensor_along_last_dim(mixed_x_layer, 3)
        ...
        if is_infer:
            ...
            ltor_mask = torch.tril(torch.ones((1, tgt_len, src_len), device=hidden_states.device)).view(1, 1, tgt_len, src_len).type(previous_type)
        converted_mask = 10000.0 * (1.0 - ltor_mask)
        attention_scores = (torch.mul(attention_scores, ltor_mask) - converted_mask).type(previous_type)
...
class GPT3Transformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layers = torch.nn.ModuleList([GPT3TransformerLayer(config) for _ in range(self.num_layers)])
...
class GPT3TransformerLanguageModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, position_ids):
        ...
        transformer_output = self.transformer(transformer_input, attention_mask)
...
class GPT3Model(PreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, position_ids=None, labels=None, **kwargs):
        ...
        attention_mask = torch.tril(torch.ones((1, 1, seq_length, seq_length), dtype=torch.long, device=input_ids.device))
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]]):
        ...
        model = cls(config)
        state_dict_file = os.path.join(pretrained_model_name_or_path, ModelFile.TORCH_MODEL_BIN_FILE)
        state_dict = torch.load(state_dict_file)
        if "state_dict" in state_dict:
            state_dict = state_dict["state_dict"]
        state_dict = {k.replace("model.language_model", "language_model"): v for k, v in state_dict.items()}
    ...
    def streaming_generate(self, tokens, temperature=1.0, **kwargs):
        ...
        is_generation_done = torch.zeros(batch_size, dtype=torch.uint8, device=tokens.device)
