...
from transformers.utils import ModelOutput, add_code_sample_docstrings, add_start_docstrings, add_start_docstrings_to_model_forward, logging
...
from .qwen_generation_utils import HistoryType, StopWordsLogitsProcessor, decode_tokens, get_stop_words_ids, make_context
...
try:
    ...
    print("Warning: import flash_attn fail, please install FlashAttention " "https://github.com/Dao-AILab/flash-attention")
...
class QWenMLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.w1 = nn.Linear(config.hidden_size, config.ffn_hidden_size // 2, bias=not config.no_bias)
        self.w2 = nn.Linear(config.hidden_size, config.ffn_hidden_size // 2, bias=not config.no_bias)
...
class QWenPreTrainedModel(TorchModel, PreTrainedModel):
    ...
    def _instantiate(cls, **kwargs):
        ...
        if model_dir is None:
            ...
            model = super(Model, cls).from_pretrained(pretrained_model_name_or_path=model_dir, **kwargs)
            ...
            self.inv_freq = 1.0 / (
                base**(torch.arange(
                    0, self.dim, 2, device=self.inv_freq.device).float()
                       / self.dim))
            """
            self.inv_freq = torch.arange(0, self.dim, 2, device=self.inv_freq.device).float() / self.dim
